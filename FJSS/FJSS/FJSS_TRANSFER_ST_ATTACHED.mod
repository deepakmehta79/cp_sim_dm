 ...
 *********************************************/
//MACHINE DEPENDENT BUT SEQUENCE INDEPENDENT ATTACHED SETUP TIMES
//ATTACHED : JOB MUST BE ON THE RESOURCE TO PERFORM SETUP, (OR SETUP IS ATTACHED TO PROCESSING TIME) 
...
Cmax>=start[i]+sum(o in operation:o.job==i)((time[o.job][o.machine]+st[o.job][o.machine])*assigned[o]);//makespan
start[i]>=sum(o in operation:o.job==i)(readytime[o.machine]*assigned[o]);//starting time is greater than ready time. SETUO TIME IS NOT INCLUDED BECAUSE OF DETACHED SETUPS

};
forall(i in jobs,j in jobs:con[i][j]>0){//precedence relation
	start[j]>=start[i]+sum(o in operation:o.job==i)((time[o.job][o.machine]+st[o.job][o.machine])*assigned[o])+
...
forall(i in operation,j in operation:((i.machine==j.machine)&&(i.job<j.job))){//disjunctive relations
	...
	start[i.job]>=start[j.job]+time[j.job][j.machine]+st[j.job][j.machine]*assigned[i]-M*(1-edge[<j,i>]);
	start[j.job]>=start[i.job]+time[i.job][i.machine]+st[i.job][i.machine]*assigned[j]-M*(1-edge[<i,j>]);
}
...
{gantts}gantt={<a,b,start[b],time[b][a],st[b][a],start[b]+time[b][a]+st[b][a]>|a in machines,b in jobs:(time[b,a]>0)&&(assigned[<a,b>]==1)};
