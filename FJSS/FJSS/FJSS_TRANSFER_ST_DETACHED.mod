...
//MACHINE DEPENDENT BUT SEQUENCE INDEPENDENT DETACHED SETUP TIMES
//DETACHED : JOB DOE NOT HAVE TO BE ON THE RESOURCE TO PERFORM SETUP, 
//IF RESOURCE IS AVAILABLE SETUP CAN START WITHOUT WAITING THE ACTUAL JOB 
...
start[i]>=sum(o in operation:o.job==i)((readytime[o.machine]+st[o.job][o.machine])*assigned[o]);//starting time is greater than ready time plus setup time
...
forall(i in operation,j in operation:((i.machine==j.machine)&&(i.job<j.job))){//disjunctive relations
	...
	start[i.job]>=start[j.job]+time[j.job][j.machine]+st[i.job][i.machine]*assigned[i]-M*(1-edge[<j,i>]);
	start[j.job]>=start[i.job]+time[i.job][i.machine]+st[j.job][j.machine]*assigned[j]-M*(1-edge[<i,j>]);
}
